...
class TLSServerKeyExchangeECDH(TLSServerKeyExchange):
    """
    TLS Elliptic Curve Diffie-Hellman Server Key Exchange Message
    ...
    def __init__(self, curve_type: ECCurveType, named_curve_type: NamedCurveType,
                 public_key: bytes, signature: bytes, server_params: Optional[bytes] = b"",
                 security_params: Optional[SecurityParameters] = None, **kwargs):
        if security_params and server_params:
            server_params = security_params.client_random + security_params.server_random + server_params
        else:
    ...
    def server_public_key(self):
        return self._server_key_exchange_public_key
    ...
    def parse(frame: bytes, tls_version: Optional[TLS] = None,
        ...
        #   ECPoint public;
        # } ServerECDHParams;
        server_params = frame[0: server_params_len]

        return TLSServerKeyExchangeECDH(curve_type, named_curve_type, pubkey, signature,
                                        server_params=server_params, **kwargs)

    def pack(self, payload: Optional[Union[bytes, None]] = None) -> bytes:
        raise NotImplementedError("TODO: Not yet implemented since we are functioning as a client")

    @staticmethod  # TODO:Move to tls_named_curve.py
    def named_groups_import(named_curve: NamedCurveType, pubkey: bytes) -> Union['X448PublicKey', 'X25519PublicKey', 'EllipticCurvePublicKey']:
        if named_curve == NamedCurveType.X488:
            return x448.X448PublicKey.from_public_bytes(pubkey)

        elif named_curve == NamedCurveType.X25519:
            return x25519.X25519PublicKey.from_public_bytes(pubkey)
        else:
            curve = get_named_curve_by_type(named_curve)
            if curve:
                try:
                    return ec.EllipticCurvePublicKey.from_encoded_point(curve, pubkey)

                except AttributeError:
                    point = ec.EllipticCurvePublicNumbers.from_encoded_point(curve, pubkey)
                    public_number = point.public_numbers()
                    return public_number(default_backend())


class TLSServerKeyExchangeDH(TLSServerKeyExchange):
    """
    TLS Diffie-Hellman Server Key Exchange Message
...
class TLSServerKeyExchangeRSA(TLSServerKeyExchange):
    """
    TLS RSA Server Key Exchange Message
